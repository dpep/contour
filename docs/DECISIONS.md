...
50 warm scoped `similar` runs in 6.15 s against 6.13 s, and 132–162
involuntary context switches against 111–119 over five runs each, which is the
threads themselves — while one blob parse costs about **0.3 ms** of CPU (17,639
blobs, 5.26 s of user time). A fixed cost that small pays for itself at
...
**Postscript, after DEC-040: the same 6.1 ms is now 3%, because the denominator
moved.** Scoping the unit and signature reads took a warm scoped `similar` at
132k units from 460 ms to 210 ms, so the vector read's share roughly tripled
without the read itself changing by a tenth of a millisecond. The ruling stands
and the numbers behind it are restated rather than left stale: 6.1 ms of 210 ms
is still not what a new on-disk artifact with a version stamp, a staleness rule
and a rebuild path is for. The mmap arc is not re-litigated here — only its
arithmetic is kept honest, and the direction it points has not changed, because
the two things now above the vector read on a warm scoped query are `refresh`
and the ONNX session load, and neither is a read of the index.
...
**Taken, in DEC-040.** The second spelling is generated by `paths` and pinned by
a test that runs both, and the two reads are 0.5 ms and 0.7 ms where they were
120 ms and 89 ms. `read summaries` is still the one left.
...
## DEC-040 — A scope is applied by the database, and its rule has a twin with a test

DEC-033 and DEC-036 both stopped at the same sentence: `Store::units` reads
every unit in the checkout and filters by path in Rust, and moving that filter
into SQL "means writing `paths::under`'s rule a second time, in a different
language, and two spellings of 'is this path in scope' that agree today is the
exact shape of a bug that arrives later." That was the right thing to decline
*blind*. DEC-038 then measured what declining it cost: `Store::units` and
`Store::signatures`, the two unscoped whole-table reads, are 45% of a warm
scoped query, and the bill grows with the corpus rather than with the question —
the same 365-unit query is 0.46 s at 132k units and 0.69 s at 265k.

So the trade is taken, and what makes it safe is that the second spelling is
**generated by the first module and pinned by a test that runs both**.

### As built

`paths::under_sql(prefix)` sits beside `paths::under` and returns the SQL
predicate plus the value to bind, or `None` when the scope is everything —
which is no predicate at all, so a whole-checkout read is the read it always
was and no caller has to spell "everything" as a path. Both functions get their
answer to "what does this prefix actually name" from one private `named()`, so
the empty and `.` cases have a single definition rather than two that agree.

The predicate goes on `file.path`, not on the units. One row per path is the
smallest place the question can be asked, and the file table is thousands of
rows where the unit table is hundreds of thousands.

### It is a range, not a `LIKE`, and that is not a micro-optimization

The obvious spelling is `f.path = ?1 OR f.path LIKE ?1 || '/%'`, and it is
wrong twice. A path may contain `%` and `_` — both LIKE wildcards — so the
prefix has to be escaped, and the agreement test catches exactly that: with the
unescaped spelling, a scope of `a%b` also claims `a_b` and `axb`. Adding
`ESCAPE` fixes the meaning and forfeits the plan, because SQLite's
LIKE-to-range optimization is off for any `LIKE` that carries an `ESCAPE`
clause. The escaped spelling can only ever scan.

The range needs no escaping, because nothing in it is a pattern:

```sql
f.path >= ?2 AND f.path < ?2 || '0'
  AND (f.path = ?2 OR f.path >= ?2 || '/')
```

`/` is 0x2f and `0` is 0x30, so under BINARY collation the strings beginning
`p/` are exactly those in `[p‖'/', p‖'0')`. Both lines are load-bearing and the
test proves each is: the bounds alone also admit `p.rb` and `p-old`, since every
byte below `/` sorts between `p` and `p‖'/'`; the residual alone leaves the
planner nothing to seek with.

### What `EXPLAIN QUERY PLAN` says, rather than what it was assumed to say

Measured on the 132k-unit corpus, and the two spellings differ:

| predicate on `f.path` | plan for `file` | the query |
| --- | --- | ---: |
| none (whole checkout) | `SEARCH f USING PRIMARY KEY (checkout_id=?)` | 84 ms |
| `= ?2 OR (range)` — a bare disjunction | `SEARCH f USING PRIMARY KEY (checkout_id=?)` | 2.5 ms |
| bounds `AND` residual, as built | `SEARCH f USING PRIMARY KEY (checkout_id=? AND path>? AND path<?)` | 0.4 ms |

A disjunction leaves the planner with the checkout constraint alone, so it
still visits every file row in the checkout — 17,721 of them here, and that
would have been the answer the task predicted: a scan of the small table
replacing materialization of the large one, and a real win at 30×. Writing the
bounds as a conjunction and the boundary as a residual is what turns it into a
seek on `file`'s existing `(checkout_id, path)` primary key. The difference is
2 ms today and is the difference between a scoped query that grows with the
checkout and one that does not, which is the whole point.

### The test is the reason a second spelling is allowed to exist

`paths::the_sql_twin_partitions_paths_the_way_rust_does` drives a set of
deliberately awkward paths — boundary prefixes, shared stems, `%` and `_` in
names, a `.` in a directory name, nesting, the byte neighbours of `/`, and the
root scope — through `under` and through the shipping predicate against a
scratch table, and asserts the two keep the same rows for every prefix. It is
the shipping string, aliased `f`, so the test cannot drift from what
...
**No read-through to abandon back to, unlike [`Store::vectors`] — and it does
cost something.** The crossover DEC-033 built for the vectors is real here too,
and it runs the other way at the top end. Measured on the 132k-unit corpus: a
read-through is 89 ms for 747,145 rows (0.12 µs a row) and a seek per body is
147 ms for 70,789 bodies (2.1 µs a body), so the read-through wins once the
request reaches about a **seventeenth** of the table's rows — which a
whole-checkout `similar` does, because it asks for every body in the checkout.

So an *unscoped* `similar` or `search` pays about **60 ms more** than it did:
0.92 s → 0.98 s at 132k units, 1.65 s → 1.66 s at 265k. `dupes --near` does not
move at all (8.8 s → 8.6 s, inside the spread), because its cost is the
combinatorics. Against that, the scoped read — the one this change is about —
goes **89 ms → 0.7 ms**.

**That is accepted rather than bought back**, and the arithmetic above is here
so the ruling is falsifiable. Buying it back means a second read strategy with a
second measured constant: the vector table's `SCAN_RATIO` of 3 does not transfer,
because a signature row is 32 bytes where a vector row is 1.5 KB. Two tuning
constants in two places, one of which will drift the first time nobody
re-measures, to recover 6% of the query DEC-030 and DEC-032 exist to steer
people away from.

**And it is the wrong fix anyway**, which is the better reason. The tier does
not want "the candidates' bodies"; it wants "bodies that share a sub-shape with
mine", which is a much smaller set and is what the `signature_subtree` index is
for. Asking that question directly would make the unscoped case *faster* than it
was rather than 60 ms slower, and it is a change to what `near::neighbors` asks,
not to how the store answers. Recorded as the next thing for whoever picks up
the near tier.

### What it bought, measured

Warm, on the synthetic corpus at two sizes, median of five with a cold run
discarded, and with every answer compared before and after — **byte-identical
at both sizes and both scopes**. `docs/PLAN.md` has the full tables and the
phase split.

| warm query about one directory (365 units) | 132,534 units | 265,068 units |
| --- | ---: | ---: |
...
`read units` goes 120.5 ms → 0.5 ms and `read signatures` 88.7 ms → 0.7 ms, so
index I/O falls from 46% of the warm query to 3.5%, and the four reads together
are flat across the two corpus sizes: 7.4 ms and 8.2 ms.

**The prediction was half right, and the half that failed is worth naming.** A
scoped query does not become flat — it goes 0.22 s to 0.32 s — and every
millisecond of the difference is `refresh`, the `git ls-files` and map-key fold
that DEC-018 pays to keep an answer from being silently stale. That grows with
the *checkout*, not with the corpus, and it is now the largest phase of a warm
scoped query at 54%, with the fixed ONNX session load at 38% behind it. Set
those two aside and a scoped query is 16 ms at 132k units and 18 ms at 265k.

### Which callers stay unscoped, deliberately

The whole-checkout readers pass `None` and get the query they always had:
`eval` (it scores the corpus, and a scope would be scoring something else),
`summary::coverage` and `summary::answerable` (`--status` reports on a
checkout), and `contributed` (a contribution names a unit, and refusing one
because the session is standing in the wrong directory would be a new failure
mode). `index` has no scope at all, for DEC-030's reason. None of them is
contorted through a scope of "everything": `None` is a distinct case in the
helper, and it emits no predicate.

`search::in_scope`, which existed only to apply the filter the store now
applies, is gone rather than kept as a pass-through.
