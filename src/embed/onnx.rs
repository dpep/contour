//! VENDORED from gqls `src/semantic/embed/onnx.rs` (which took it from ae),
//! MIT, same author. The only changes are this header and the `GQLS_` →
//! `CONTOUR_` environment prefix.
//!
//! The real embedder: `all-MiniLM-L6-v2` (int8-quantized ONNX) via ONNX
//! Runtime, with mean pooling over the token embeddings.
//!
//! Resolution when no explicit `--model` is requested: `$CONTOUR_MODEL_DIR` (a
...
// `dims()` serves gqls's startup log line; contour discloses `kind()` instead.
// Kept so a sync stays a re-copy.
#![allow(dead_code)]
...
/// all-MiniLM-L6-v2). Override with `--embed-model <dir | .onnx | org/name>`.
