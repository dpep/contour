//! VENDORED from gqls `src/semantic/mrl.rs` (which took it from ae), MIT, same
//! author. Verbatim apart from this header and `pub(crate)` → `pub`.
//!
//! Vector truncation to a fixed prefix width.
//!
...
//! the relevance floor in `crate::search` sits in the middle of its gap.
//!
