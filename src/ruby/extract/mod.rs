//! VENDORED from trekr `src/extract/` (~/code/lib/rust/trekr, MIT, same
//! author). Keep edits mechanical so an upstream sync stays a re-copy: the
//! only changes are this header, the `dead_code` allow, and `crate::core` →
//! `super::facts`.
//!
//! Blob → facts. A pure function of bytes, and nothing else. Semantics are
//! lifted from Shopify's Rubydex (MIT); trekr's `docs/ruby-behaviors.md` is
//! the conformance spec.
//!
//! Traversal is Prism's `Visit` trait with a scope stack on `self`: push the
//! frame, call the free `ruby_prism::visit_*` to descend, pop.
//!
//! contour reads only `Facts::defs` (see `super::units`); the rest is
//! computed and discarded. See `super::facts` for why that is the cheaper
