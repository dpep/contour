//! VENDORED from trekr `src/core/mod.rs` (~/code/lib/rust/trekr, MIT, same
//! author). Keep edits mechanical so an upstream sync stays a re-copy.
//!
//! Trekr's fact vocabulary — what one Ruby blob says about itself. Every type
//! here is a pure function of the blob's bytes: no paths, no repo identity, no
//! cross-file resolution.
//!
//! Deleted from upstream, as three self-contained blocks:
//!   - `Oid` and `mod paths` — contour has its own in `crate::core` / `crate::paths`.
//!   - `Facts::surface()` and its tests — trekr's tree-rebuild probe; contour
//!     has no tree layer.
//!
//! Everything else is verbatim, including facts contour never reads (calls,
//! const_refs, assigns, ancestry). Trimming the extractor to stop computing
//! them would be a 2,000-line edit against carefully-tested Rails macro
//! handling, to save a parse-time allocation nobody has measured. The
//! module-wide `dead_code` allow below is the price, and it is scoped to
//! vendored code whose lint premise — that this crate owns it — does not hold.
#![allow(dead_code)]
